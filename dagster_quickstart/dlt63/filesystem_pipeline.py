...
def read_csv_with_duckdb() -> None:
    ...
    input_data_path = "Users/seanmcfall/workspace/dagster-dlt-dbt-63-demo/input_data/"
    # load all the CSV data, excluding headers
    met_files = readers(
        bucket_url = "{}medical/".format(input_data_path) + , file_glob="*.txt"
    ).read_csv_duckdb(chunk_size=1000, header=True)

    load_info = pipeline.run(met_files, loader_file_format="parquet", table_name = "foodog")
